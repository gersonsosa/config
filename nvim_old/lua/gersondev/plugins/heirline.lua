  ...
  config = function()
    ...
    local file_icon = {
      init = function(self)
        ...
        self.icon, self.icon_color = Snacks.util.icon(filename)
        self.icon_color = Snacks.util.color(self.icon_color)
      end,
      provider = function(self)
    ...
    local lsp_progress = {
      ...
      update = {
        "LspProgress",
        callback = vim.schedule_wrap(function(self, ev)
      ...
      provider = function(self)
        if self.done then
          ...
          return table.concat({
            self.spinner[math.floor(vim.uv.hrtime() / (1e6 * 80)) % #self.spinner + 1],
            escape_statusline(vim.lsp.status()),
          }, " ")
        end
      end,
    ...
    local lsp_server_list = {
      ...
      provider = function()
        ...
        for _, server in pairs(vim.lsp.get_clients()) do
          ...
          table.insert(names, "" .. icon .. " " .. server.id .. "")
        end
