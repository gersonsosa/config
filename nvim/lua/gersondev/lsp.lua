local setup_lsp_keymap = function(_, bufnr)
  ...
  vim.keymap.set("n", "gr", function() telescope_builtin.lsp_references() end,
    { desc = "[G]o to references", buffer = bufnr })
  vim.keymap.set("n", "gy", function() telescope_builtin.lsp_type_definitions() end,
    { desc = "[G]o to t[y]pe definition", buffer = bufnr })
  vim.keymap.set("n", "gi", function() telescope_builtin.lsp_implementations() end,
    { desc = "[G]o to [i]mplementation", buffer = bufnr })
  vim.keymap.set("n", "<leader>ds", function() telescope_builtin.lsp_document_symbols() end,
    { desc = 'List [d]ocument [s]ymbols', buffer = bufnr })
  vim.keymap.set("n", "<leader>ws", function() telescope_builtin.lsp_workspace_symbols() end,
    { desc = "List [w]orkspace [s]ymbols", buffer = bufnr })
end
